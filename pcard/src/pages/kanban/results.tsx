...
export default function Results({
    ...
    async (patch: Partial<ResultRow>) => {
      ...
      try {
        ...
        const body: any = {
          ...
          certMiss: patch.certMiss ? Number(patch.certMiss) : "",
          fps: patch.fps ? Number(patch.fps) : "",
          cfnPreview: patch.previewCfn ? Number(patch.previewCfn) : "",
          cfnFinal: patch.finalCfn ? Number(patch.finalCfn) : "",
          privateFlag:
  ...
  const columns = useMemo(
      ...
      { title: "Stage", dataIndex: "stageLabel", width: 180 },
      {
        ...
        render: (v: any, r: ResultRow) => (
          <Input
            ...
            onChange={(e) => onCellChange(r.key, { certMiss: e.target.value })}
          />
        ),
      },
      {
        ...
        render: (v: any, r: ResultRow) => (
          <Input
            value={v || ""}
            disabled={effectiveLocked || !!r.invResFlag}
            onChange={(e) => onCellChange(r.key, { fps: e.target.value })}
          />
        ),
      },
      {
      ...
      },
      {
        ...
        render: (v: any, r: ResultRow) => (
          <Input
            ...
            disabled={effectiveLocked || !!r.invResFlag}
            onChange={(e) =>
              onCellChange(r.key, { previewCfn: e.target.value })
            }
          />
        ),
      },
      {
        ...
        render: (v: any, r: ResultRow) => (
          <Input
            value={v || ""}
            disabled={effectiveLocked || !!r.invResFlag}
            onChange={(e) => onCellChange(r.key, { finalCfn: e.target.value })}
          />
        ),
      },
      {
    ...
    [effectiveLocked, gradeFromCertMiss, onCellChange],
  );

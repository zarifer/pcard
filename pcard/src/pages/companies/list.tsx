                ...
                {filtered.map(company => (
                    <Col xs={24} sm={12} md={8} lg={6} key={company.id}>
                        <Card
                            hoverable
                            actions={[
                                <ShowButton resource="companies" recordItemId={company.id} key="view" />,
                                <EditButton resource="companies" recordItemId={company.id} key="edit" />,
                                <DeleteButton resource="companies" recordItemId={company.id} key="delete" />,
                            ]}
                        >
                            <Card.Meta
                                avatar={<Avatar src={company.logo} size={48}>{company.name?.charAt(0)}</Avatar>}
                                title={<Typography.Title level={5} style={{ margin: 0 }}>{company.name}</Typography.Title>}
                                description={
                                    <>
